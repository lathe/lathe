...
(mr (rule expbysquare (base exp)
      ; This condition subsumes the condition for the 'one
      ; contribution on purpose, so that we can test 'prec. Since this
      ; contribution comes later in the code, it ends up farther
      ; toward the beginning of the contribution stack, and ultimately
      ; this rule would be tried before the 'one rule
      ; (catastrophically) if not for the presence of
      ; [iso (map _ '(name label)) '(expbysquare one)] in the 'prec
      ; line below.
...
(mr (rule expbysquare (base exp) diabolical
      (err:+ "Somehow the 'diabolical contribution to 'expbysquare "
             "was used.")))
